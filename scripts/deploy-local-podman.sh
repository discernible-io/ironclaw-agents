...
build_images() {
  # Prefer layered builds + Dockerfile.reborn cache mounts (cargo/pnpm).
  # Avoid --no-cache unless intentionally forcing a cold rebuild.
  echo "==> Building ${REBORN_IMAGE} (Dockerfile.reborn, layered)"
  podman build --layers -f "$REPO_ROOT/Dockerfile.reborn" -t "$REBORN_IMAGE" "$REPO_ROOT"
