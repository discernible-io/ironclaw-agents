...
cmd_build_image() {
  ...
  # --no-cache unless deliberately busting. Dockerfile.reborn also uses
...
cmd_identyclaw_init() {
  ...
  if command -v npm >/dev/null 2>&1; then
    ...
    if [[ -n "$rodit_src" && -d "$rodit_src" ]]; then
      ...
      (cd "$ROOT/deploy/identyclaw" && npm install --omit=dev "@rodit/rodit-auth-be@9.16.2") || true
    fi
